...
public class BigInt implements Comparable<BigInt> {
    ...
    BigInt(int value) {
        if (value < 0)
            System.out.println("Caution: negative numbers not currently implemented");
    ...
    public static String calculate(String expression) {
        ...
        switch (operator) {
            case 'x':
                result = BigInt.multiply(x, y);
                break;
            case '+':
                result = BigInt.add(x, y);
                break;
            case '-':
                result = BigInt.subtract(x, y);
                break;
            case '/':
                result = BigInt.divide(x, y);
                break;
            case '^':
                result = BigInt.power(x, y);
                break;
            default:
    ...
    // does not support negative exponents, but would be easy to add because we are
    // using natural numbers
    public void power(int exp) {
        this.setValue(power(this.getValue(), exp));
    }
    ...
    private static String multiplyByInt(int x, String num, int powerOf10, int overflow, StringBuilder sb) { // recursive
                                                                                                            // exit
                                                                                                            // cond.
                                                                                                            // will
                                                                                                            // return
